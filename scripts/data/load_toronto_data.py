...
class DataPipeline:
    ...
    def _cleanup_deprecated_tables(self) -> None:
        """Drop known deprecated mart_toronto tables from previous consolidations.

        Uses explicit list of deprecated tables to ensure cleanup works
        regardless of dbt manifest state. Does not rely on manifest comparison.
        """
        from sqlalchemy import text

        # Known deprecated tables from previous consolidations
        deprecated_tables = {
            "mart_neighbourhood_overview",  # Consolidated into livability
            "mart_neighbourhood_demographics",  # Consolidated into people
            "mart_neighbourhood_amenities",  # Consolidated into people
            "mart_neighbourhood_housing_rentals",  # Consolidated into housing
        }

        try:
            dropped_count = 0
            with get_session() as session:
                for table_name in sorted(deprecated_tables):
                    try:
                        session.execute(
                            text(
            ...
            if dropped_count > 0:
                logger.info(f"  Cleanup: removed {dropped_count} deprecated table(s)")
